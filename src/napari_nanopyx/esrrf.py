...
@magic_factory(
    ...
    reconstruction_order={
        ...
        "value": "AVG",
        "choices": [("Average", "AVG"), ("Variance", "VAR"), ("Autocorrelation", "TAC2")],
        "label": "Shift Calculation Method"}
    )
def generate_esrrf_image(
        viewer: Viewer,
        img: Image, magnification: int,
        sensitivity: float,
        radius: float,
    ...
    for i in range(img.data.shape[0] // frames_per_timepoint):
        ...
        output_stack.append(calculate_eSRRF_temporal_correlations(eSRRF_generator.calculate()[0], reconstruction_order))
