...
@magic_factory(
    ...
    n_benchmark_runs={
        "label": "Number of benchmark runs (min. 3 recommended)"
    },
    img_dims={"label": "Image Dimensions (px)", "value": 100, "min": 50},
    shift={"label": "Image Shift (px)", "value": 0, "min": 0},
    magnification={"label": "Magnification (int)", "value": 5, "min": 1},
    rotation={"label": "Rotation (degress)", "value": 15.0, "min": 0.0},
    conv_kernel_dims={
...
def benchmark_nanopyx(
    viewer: napari.Viewer,
    n_benchmark_runs: int = 3,
    img_dims: int = 100,
    shift: int = 2,
    magnification: int = 5,
    rotation: float = 15.0,
    conv_kernel_dims: int = 23,
):
    """
    ...
    img_int_4grads = np.random.random(
        (1, img_dims * magnification, img_dims * magnification)
    ).astype(np.float32)
    img_grads = np.random.random(
    ...
    pbr.set_description(
        "Benchmarking Nearest-neighbour Shift, Scale and Rotate"
    )
    for i in range(n_benchmark_runs):
        nn_ssr.benchmark(
            img, shift, shift, magnification, magnification, rotation
    ...
    napari.utils.notifications.show_info(
        "NanoPyx bencharking has finished, ready to zoom!"
    )
